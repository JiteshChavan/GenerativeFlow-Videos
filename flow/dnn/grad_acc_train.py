...
from flow.dnn import create_dnn
...
def main(args):
    ...
    for current_step in range(init_step, total_steps):
        ...
        for micro_step in range(grad_acc_steps):
            ...
            grad_sync_ctx = DNN.no_sync() if  micro_step < (grad_acc_steps - 1) else contextlib.nullcontext()
            with grad_sync_ctx:
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    loss = flow.training_step(x, y)
                # addition of gradients corresponds to SUM in objective we want MEAN over effective batch size
...
if __name__ == "__main__":
    ...
    parser.add_argument("--grad-acc-steps", type=int, required=True)
